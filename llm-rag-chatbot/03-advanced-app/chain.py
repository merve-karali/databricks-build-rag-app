...
query_rewrite_template = """Based on the chat history below, generate a query to search for relevant candidate profiles from the provided resumes. The query should be in natural language and reflect the user's most recent question or intent. The external data source uses similarity search in a vector space to retrieve candidate profiles, so the query should focus on the key skills, experience, and qualifications related to the job role. Answer with only the query. Do not provide any explanations or additional context.
